...
<!-- ============================================================== -->
	...
	<div class="modal fade" id="cd_modalName" tabindex="-1"
		...
		<!-- ============================================================== -->
		var gridColums = [];
		var btm_Scroll = false;   		// scrollX 끔 → 표가 폭 100% 채움
		var auto_Width = false;   		// 열 너비 자동 계산 off → 컬럼 폭 분배
		  ...
		  if (!$.fn.DataTable.isDataTable('#' + cd_tableName.id)) {
		   	cd_dataTable =  $('#' + cd_tableName.id).DataTable({  // 올바르게 닫힌 선택자
		            ...
		            columns: [
		            	...
		            	{ title: "구분명칭",    data: "dtlCodeNm_one"  ,  className: "text-center", defaultContent: ''},
		...
		function cdontLoad(data, callback, settings) {
			$('#' + tableName.id).on("click", "tr", function () {
			    ...
			    if (codeCd_one) {
			        ...
			        $.ajax({
			            url: ctx + "/base/commDtlList.do", // 실제 서버 엔드포인트 입력
			            type: "POST",
