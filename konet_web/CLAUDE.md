...
- **화면 4탭**(모두 JSP 내장 JS. 서버 무변경 — 기존 조회 API 2개를 합칠 뿐). **집계 2 × 원본 2 가 아니라 축 3 + 원본 1** 로 정리됨(2026-07-22, ②와 ④가 겹쳐 보인다는 사용자 지적 → ④를 사업장축으로 전환):
  ...
  ④ **정산서 원본(엑셀)** = 정산서 행 그대로 + 출고수량 열
  - 한쪽 원천이 통째로 없으면 빈 표 대신 **왜 비었는지 안내**(`_ohEmptyRow`) — 정산서만 없는 날/발주현황표만 없는 날을 구분해 문구가 달라진다.
  - **출고장 묶음은 대시보드(`CLOSE_DCGROUP` 오산센터)를 쓰지 않고 개별 출고장 1:1**(사용자 확정 2026-07-22). 이유: 정산서가 출고장별로 따로 오고(왜관도 자기 정산서 8행), `DC_CD`↔거래처가 1:1(E200 왜관=00275 / E700 오산=00278)이라 청구·수금이 갈라진다. 대시보드 묶음은 물류 동선용. **총합계는 같고 줄 수만 다르다**(매출마감 3줄 vs 이 화면 5~7줄).
...
- **기간 기준 = 납품일자(DLV_DT = 발주일자)로 양쪽 통일.** 출고 API(`selectShipoutMst`)는 SHPOUT_DT 로만 걸리는데 먼 지역은 하루 당겨 출고하므로 **±7일 넉넉히 읽어 화면에서 DLV_DT 로 재필터**(`ohQuery`).
