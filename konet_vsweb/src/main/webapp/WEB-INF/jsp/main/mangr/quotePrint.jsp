...
<style>
  *{ box-sizing:border-box; }
  ...
  .items td.wrap{ white-space:normal; line-height:1.25; font-size:11.5px; }
...
<c:otherwise>
  <c:set var="has2" value="${not empty mst.price2Nm}"/>
  <h1>견 적 서</h1>
  <table class="hd">
  ...
  <table class="items">
    ...
    <c:forEach var="it" items="${items}">
      <tr><td class="l wrap">${it.prodNm}</td><td class="l wrap">${it.spec}</td>
          ...
          <td class="l wrap">${it.remark}</td></tr>
    </c:forEach>
    ...
    <tfoot class="foot">
      <tr><td class="c" colspan="3">합 계 (부가세 별도)</td><td></td><td></td><td class="r"><fmt:formatNumber value="${mst.supplyAmt}" pattern="#,##0"/></td><c:if test="${has2}"><td></td><td></td></c:if><td></td></tr>
    </tfoot>
  </table>
