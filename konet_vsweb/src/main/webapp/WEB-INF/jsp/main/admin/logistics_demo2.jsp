...
<script type="text/javascript">
    ...
    if (f) {
      ...
      /* ★세션이 끊긴 채 눌러 '로그인 화면'이 들어가 있으면 다시 부른다 (2026-08-06).
        ...
        try {
          ...
          /* ★다시 보일 때 그 화면에 알려 준다 (2026-09-13 「기존 정보 바꿔도 매입·판매등록에 적용 안 돼 로그아웃했다 들어가야」)
  ...
  function _salesNote(){ return _salesQNote()+_salesUnNote()+(window.closeNoCostNote?closeNoCostNote(_salesRows,'sales'):''); }   /* 매입가 없는 품목 경고(2026-09-16 P2-f) — 함수는 logi-oh.js */
  ...
  function statRenderTab(){
    ...
    if(tab==='item'){
      ...
      sum.innerHTML='총 <b>'+agg.length.toLocaleString()+'</b>품목 · 매출 <b>'+_cnum(gS)+'</b> · 매출총이익 <b>'+_cnum(gM)+'</b>'+(window.closeNoCostNote?closeNoCostNote(_statRows,'stat'):'');   /* 매입가 없음 경고(2026-09-16) — logi-oh.js */
    ...
    if(tab==='zone'){
      ...
      sum.innerHTML='총 <b>'+L1.length+'</b>대표출고장 · 매출 <b>'+_cnum(gS)+'</b> · 매출총이익 <b>'+_cnum(gM)+'</b>'+(window.closeNoCostNote?closeNoCostNote(_statRows,'stat'):'');
      ...
      sum.innerHTML='총 <b>'+groups.length+'</b>사업장 · 매출 <b>'+_cnum(gS)+'</b> · 매출총이익 <b>'+_cnum(gM)+'</b>'+(window.closeNoCostNote?closeNoCostNote(_statRows,'stat'):'');
...
<div class="logi-wrap">
  ...
  <nav class="logi-side">
    ...
    <div class="sub-menu" id="sub-quote">
      <a class="mi" data-key="quoteMng" onclick="logiFrame('quoteMng','${pageContext.request.contextPath}/mangr/quoteMng.do', this)"><span class="ic">📄</span>견적서 목록</a>
  ...
  <main class="logi-main">
    ...
    <section id="panel-guide" class="panel">
      ...
      <div class="g-sec">
        ...
        <table><tbody>
          ...
          <tr><td class="m">⏰ 자동 로그아웃</td><td><b>30분 동안 아무 조작이 없으면 저절로 로그아웃</b>됩니다(29분에 빨간 예고줄 — 마우스나 키보드를 움직이면 유지). 또 <b>서버가 재배포·재기동되면</b> 그 자리에서 로그인 화면으로 보냅니다 — 옛 화면·끊긴 세션으로 계속 쓰다 저장·조회가 이상해지는 것을 막는 장치입니다. 로그인만 다시 하면 됩니다.</td></tr>
        </tbody></table>
      </div>
      ...
      <div class="g-sec">
        ...
        <table><tbody>
          <tr><td class="m">출고현황표(대시보드)</td><td>발주현황표 엑셀을 올려 <b>출고장·사업장·품목별 출고량</b>을 작성합니다. 매출마감·재고차감의 원천.
            ...
            <b>· 이전 자료 알림</b> 마지막에 올린 것보다 출고일자가 앞서면 알려만 주고 <b>막지 않습니다</b>.<br>
            <b>· 가마감 vs 현 발주</b> 엑셀 헤더에 <b>입고장·존·버스번호·RT순번·수량·배송구분·특기사항</b> 이 두 번 나옵니다 — 
            <b style="color:#c0392b">현 발주를 쓰고</b>, 그 칸이 비어 있으면 가마감 칸을 씁니다. 가마감을 안 탄 직송 행은 가마감 칸이 통째로 비어 있어
            종전에는 <b>수량 0·배송구분 빈값</b>으로 들어갔습니다(2026-08-30 수정).<br>
            <b>· 입고장이 비면</b> 그 행은 번호 없이 <b>물류센터 자체</b>를 출고장으로 잡습니다(예: 평택물류센터1~4 → <b>평택물류센터</b>).<br>
            <b>· 배송/직송 구분</b> 직송 행(존·배송구분='직송')은 배송 입고장 줄에 안 섞고 <b>「○○물류센터 직송」</b> 줄로 따로 묶입니다(센터 블록 맨 아래 · 두 대시보드·엑셀출력 공통). 
            오산센터 묶음 안에서도 물류센터마다 <b>센터 합계 줄</b>을 따로 보여줍니다(2026-08-30).</div></td></tr>
      ...
      <div class="g-sec">
        ...
        <table><tbody>
          ...
          <tr><td class="m">판매 등록 / 수금 등록</td><td>정산서 밖의 <b>직접 판매</b> 전표와 <b>수금</b> 전표. 오른쪽에 그 거래처 원장이 함께 뜹니다. <b>[💬 카톡 주문 가져오기]</b> — 카톡 주문 글을 오른쪽 서랍에 붙여넣으면 거래처 ▸ 품목 ▸ 수량으로 갈라 보여 주고, 체크한 줄을 <b>[→ 판매등록으로]</b> 넘겨 명세에서 확인한 뒤 <b>기존 [저장]</b>으로 저장합니다(서랍에서 직접 저장하지 않음). 카톡 원문은 판매메모·줄 비고에 남습니다.
            <br><b>💬 카톡 주문 가져오기</b>(2026-09-04) — 카톡으로 온 주문 글을 그대로 붙여넣으면 <b>거래처 ▸ 품목 ▸ 수량</b>으로 갈라 표로 보여 줍니다(여러 날·여러 업체 한 번에).
            「*우리푸드」 같은 줄이 거래처, 「숫자+박스/개」 줄이 품목(<b>「복숭아자두--2」처럼 단위 없이 오면 박스</b> — 한 줄에 「*세진유퉁  복숭아자두--2 레드자몽--1 …」로 이어 와도 품목마다 갈라 줍니다), 「~보다 ~ 많게」 문장은 수량으로 넣지 않고 노란 줄로만. 우리 상품은 그 거래처 매칭코드로 <b>추정</b>만 하므로
            초록(매칭)·노랑(추정)·빨강(미연결)을 보고 고친 뒤, 거래처 줄의 <b>[→ 담기]</b>(명세로 가져가 검토 후 저장) 또는 <b>[💾 저장]</b>(거래처별 전표 바로 저장). <b>[📥 엑셀 출력]</b>은 분류 결과 전 줄.
            <br><b>🖨 거래명세표</b>(2026-09-09) — 지금 화면의 전표를 <b>A4 거래명세서</b>로. 누르면 조건 창이 뜹니다 —
            ① <b>정렬</b>(입력순서/조회번호) · <b>금액</b> · <b>단가</b> · <b>잔고</b> · <b>반전</b> · <b>박스단가</b> · <b>단가변동</b> · <b>부가세</b> 출력 여부,
            ② <b>한 장에 품목 몇 줄</b> · <b>공급자용·공급받는자용을 한 장에 모두</b> 인지 <b>두 장</b>인지(한 부만 찍기도 가능). <b>바코드는 찍지 않습니다.</b>
            품목이 넘치면 장이 늘고(1/2·2/2) 합계·이하여백은 마지막 장에만. 새 창에 그린 뒤 브라우저 인쇄로 나갑니다 — <b>저장 전 전표도</b> 그대로 찍힙니다.
            <b>공급받는자</b> 칸(상호·사업자번호·주소·<b>이메일</b>)은 <b>거래처 정보</b>에서, <b>공급자</b> 칸은 <b>회사 정보</b>에서 옵니다.
            <b>주소·품명·규격이 칸을 넘치면 글자를 한 단계 줄여 두 줄</b>로 접힙니다(줄 높이는 그대로라 장수는 안 바뀜 · 발주서 인쇄도 같음, 2026-09-10).
            <b>업태·종목·계좌·공지사항</b>은 조건 창에서 적고 <b>[💾 저장]</b>을 누르면 서버에 남아 <b>다른 PC에서도, 거래처에 보낸 명세서에도</b> 같게 찍힙니다(2026-09-09).
            <br><b>💬 카톡 · ✉ 이메일 · 🔗 링크로 보내기</b>(2026-09-09) — <b>발주서와 같은 방식</b>입니다.
            거래처가 <b>로그인 없이</b> 그 전표 하나만 보는 주소를 만들어 보냅니다(받는 쪽 화면에서 인쇄·PDF 저장 가능).
            <b>저장된 전표만</b> 보낼 수 있고, 주소는 <b>처음 보낼 때 한 번</b> 만들어져 다시 보내도 같습니다.
            <b>★위 ①②에서 고른 조건 그대로 보입니다</b>(2026-09-10) — 인쇄 방식(한 장에 모두·두 장·한 부만)·줄 수·금액·단가·부가세·반전·박스단가·정렬은 물론
            <b>잔고·단가변동</b>까지 <b>[👁 미리보기]로 본 그 모양</b>으로 나갑니다. <span style="color:#b45309">그전에 보낸 링크는 종전 모양(공급받는자용 한 부)대로 그대로 열립니다.</span>
            잔고는 <b>보낼 때의 금액</b>이 그대로 찍힙니다(나중에 열어도 안 변합니다).
            <b>[✉ 이메일]</b> 은 수신자 주소를 고르거나 넣고 <b>[저장]</b> 을 체크하면 그 주소가 <b>거래처 정보</b>에 남아 다음에 자동으로 채워집니다.
            메일 계정을 넣기 전에는 <b>이 PC 의 메일 프로그램</b>이 열립니다(웹메일만 쓰면 [Gmail 로 열기]·[📋 내용 복사]).
            서버 발송은 몇 초 걸립니다 — 누르면 단추가 <b>「⏳ 보내는 중…」</b>으로 잠기니 <b>한 번만</b> 누르세요(다시 누르면 그만큼 또 나갑니다 · 2026-09-10).
            <span style="color:#b45309">잘못 보냈으면 그 전표의 링크를 내려 달라고 하세요 — 주소를 지우면 즉시 안 열립니다.</span>
            <br><b>📨 전송이력</b>(2026-09-10, 작업 단추 줄 맨 끝 · ✖ 삭제하기 뒤) — <b>언제 · 누구에게 · 어떤 방법으로</b> 보냈는지 남습니다.
            <b>[이 전표]</b> 탭은 지금 명세서의 이력, <b>[전체 이력]</b> 탭은 기간·거래처로 훑어봅니다(최근 500건).
            <span style="color:#b45309"><b>실제로 나간 것만</b> 남습니다 — 💬 카톡 공유 · ✉ 이메일(서버 발송).</span> 🔗 링크 복사 · 📋 내용 복사 · 메일 프로그램 · Gmail 은 주소를 쥐거나 창을 연 것뿐이라 남기지 않습니다.
            메일이 <b>실패</b>하면 그 사유까지 함께 남습니다(「보냈는데 안 왔다」를 가릴 수 있는 유일한 기록).
          ...
          <tr><td class="m">매출마감</td><td>출고 × <b>납기일자 시점 단가</b> → 매출·매입·매출총이익(순마진은 마감현황에서 비용을 뺀 값). 출고장별(오산센터 등 2단)·사업장별·품목별 3탭, 품목 검색 공통. <b>토더 발주</b>는 정산서 없이 저장하면 곧 매출 — 수량 × 토더 판매가(엑셀 매입가, 부가세 포함), 근거 「토더」 · 받을 곳 = 거래처 「토더」(채권·채무·일계장에도). 반품은 토더 발주 등록의 저장 목록에서 수량을 고친다.</td></tr>
      ...
      <div class="g-sec">
        <h3>3. 매입 · 재고 관리</h3>
        <table><tbody>
          <tr><td class="m">매입 등록 / 지급 등록</td><td>매입 전표와 <b>지급</b> 전표. 오른쪽에 그 거래처 원장이 함께 뜹니다.
            <div style="margin-top:4px;color:#5a6b7a"><b>· 이전 단가와 다르면 알려 줍니다</b>(2026-09-13) 상품을 담으면 그 거래처의 <b>지난 매입 단가</b>가 들어오고, 단가를 고쳐 지난 매입과 달라지면 단가 칸에 <b style="color:#c0392b">▲</b>(오름)·<b style="color:#1d4ed8">▼</b>(내림) 표시가 붙고(마우스를 올리면 이전 단가·그 날짜), 명세 밑에 <b>지금 작업한 줄</b>의 안내줄이 뜹니다(누르면 그 줄로 갑니다 — 다른 줄은 ▲▼ 표시로만 남습니다). 지난 매입이 없으면 <b>상품 매입가</b>와 비교합니다. <b>매입분</b>으로 담은 줄은 목록 단가를 그대로 두고 다르면 표시만 합니다(<b>이전 비고</b>는 비고 칸이 비어 있으면 함께 들어옵니다). <b>값은 바뀌지 않고 저장도 막지 않습니다</b> — 알리기만 합니다.</div></td></tr>
          <tr><td class="m">발주서 관리</td><td>거래처에 보낼 <b>발주서</b>를 만듭니다 — 저장 후 <b>🖨 인쇄 · 📥 엑셀 · 💬 카톡 공유</b>(받는 쪽은 로그인 없이 발주서만 봅니다) · <b>📦 매입전환</b>(실제 들어온 날로 매입전표 생성). 매입전표·재고와는 별개입니다.
            <div style="margin-top:4px;color:#5a6b7a"><b>· 상품 넣기</b>(2026-09-07) 팝업에서 <b>줄을 누르면 한 개</b>가 그 자리에 들어가고 닫힙니다. <b>여러 개</b>는 왼쪽 <b>☑</b> 를 체크한 뒤 <b>[선택한 N개 담기]</b> — <b>체크한 차례대로</b> 줄이 생깁니다(검색어를 바꿔도 체크는 남으므로 나눠 찾아 한 번에 담을 수 있습니다). 머리글 ☑ = 지금 목록 전체.</div>
            <div style="margin-top:4px;color:#5a6b7a"><b>· 칸 폭 조절</b>(2026-09-07) 명세·발주서 목록의 <b>머리글 오른쪽 경계를 끌면</b> 그 칸이 넓어지고 좁아집니다(<b>더블클릭</b> = 처음 폭으로). 조절한 폭은 <b>이 컴퓨터에 남아</b> 다음에 들어와도 그대로입니다.</div>
      ...
      <div class="g-sec">
        ...
        <table><tbody>
          ...
          <tr><td class="m">기준정보관리</td><td><b>매입/매출 거래처</b>(회계 거래처 · 거래처리스트.xls 재업로드 · <b>DC 사용·DC율·여신한도</b>) · <b>거래처관리(사업장)</b>(배송 점포, 발주 업로드 시 자동등록) · 회사/사용자 · 공통코드.
            ...
            <span style="color:#5a6b7a">도장은 <b>거래명세서</b>(인쇄·보낸 링크) 공급자 「성명」 칸 · <b>발주서</b>(인쇄·카톡 링크) 「성명」 칸 · <b>견적서</b>(인쇄·엑셀) 「대표이사」 칸에 찍힙니다(도장을 지우면 세 문서 모두 빠집니다). <b>기능</b>은 새 거래처·새 상품의 첫 값(부가세·DC율·과세), 판매·매입 명세의 단가·수량 소수점·서비스·비고 칸·불량반품, 판매 저장 때 재고 부족·여신 초과 제한, 수금 기본 유형, 매입 저장 때 상품 매입단가 자동 갱신(평균단가·0원 포함 여부)을 정합니다 — 기본값은 모두 종전 동작. <b>인쇄 옵션</b>은 판매등록 [🖨 거래명세표] 조건 창의 첫 값이며 어느 PC에서나 같습니다(조건 창의 [⚙ 회사 기본값으로]로도 저장).</span> <span style="color:#5a6b7a">회사/사용자·공통코드 관리는 <b>관리자 회사</b>(코네트)에만 보입니다. 매입/매출 거래처의 [＋ 거래처 추가] 창에도 <b>최근 등록 거래처코드·거래처명</b>과 다음 코드가 나옵니다.</span>
    ...
    <%-- 재고 일괄조정 (2026-08-19 신설) — logiFrame 이 여는 아이프레임.
