      ...
      <div class="g-sec">
        ...
        <table><tbody>
          ...
          <tr><td class="m">판매 등록 / 수금 등록</td><td>정산서 밖의 <b>직접 판매</b> 전표와 <b>수금</b> 전표. 오른쪽에 그 거래처 원장이 함께 뜹니다. <b>[💬 카톡 주문 가져오기]</b> — 카톡 주문 글을 오른쪽 서랍에 붙여넣으면 거래처 ▸ 품목 ▸ 수량으로 갈라 보여 주고, 체크한 줄을 <b>[→ 판매등록으로]</b> 넘겨 명세에서 확인한 뒤 <b>기존 [저장]</b>으로 저장합니다(서랍에서 직접 저장하지 않음). 카톡 원문은 판매메모·줄 비고에 남습니다.
            ...
            초록(매칭)·노랑(추정)·빨강(미연결)을 보고 고친 뒤, 거래처 줄의 <b>[→ 담기]</b>(명세로 가져가 검토 후 저장) 또는 <b>[💾 저장]</b>(거래처별 전표 바로 저장). <b>[📥 엑셀 출력]</b>은 분류 결과 전 줄.</td></tr>
      ...
      <div class="g-sec">
        ...
        <table><tbody>
          ...
          <tr><td class="m">발주서 관리</td><td>거래처에 보낼 <b>발주서</b>를 만듭니다 — 저장 후 <b>🖨 인쇄 · 📥 엑셀 · 💬 카톡 공유</b>(받는 쪽은 로그인 없이 발주서만 봅니다) · <b>📦 매입전환</b>(실제 들어온 날로 매입전표 생성). 매입전표·재고와는 별개입니다.
            <div style="margin-top:4px;color:#5a6b7a"><b>· 상품 넣기</b>(2026-09-07) 팝업에서 <b>줄을 누르면 한 개</b>가 그 자리에 들어가고 닫힙니다. <b>여러 개</b>는 왼쪽 <b>☑</b> 를 체크한 뒤 <b>[선택한 N개 담기]</b> — <b>체크한 차례대로</b> 줄이 생깁니다(검색어를 바꿔도 체크는 남으므로 나눠 찾아 한 번에 담을 수 있습니다). 머리글 ☑ = 지금 목록 전체.</div></td></tr>
      ...
      <div class="g-sec">
        ...
        <table><tbody>
          <tr><td class="m">상품코드등록 · <br>상품(품목)관리</td><td><b>상품코드등록</b>=목록+등록 전용(하단 거래처 매칭코드) · <b>상품(품목)관리</b>=행 클릭 → 아래 <b>이력/재고</b> 4탭에서 <b>매입가·판매가 이력</b>과 <b>재고 수불(입·출고·조정·반품)</b> 등록. 둘 다 같은 마스터(TBL_PROD_MST).
            <span style="color:#5a6b7a">두 화면의 <b>추가</b> 창에는 코드 칸 아래에 <b>최근 등록 상품코드·상품명</b>과 <b>9번대 마지막 코드 → 새 코드</b>가 나옵니다. ★<b>새로 만드는 상품코드는 9로 시작</b>합니다(원천 코드 <b>1000…</b> 번대와 부딪히지 않게) — 창을 열면 새 코드가 <b>미리 들어가 있고</b>, 그대로 저장하거나 직접 쳐서 바꿀 수 있습니다(원천 코드 직접 등록도 막지 않습니다).</span></td></tr>
          <tr><td class="m">기준정보관리</td><td><b>매입/매출 거래처</b>(회계 거래처 · 거래처리스트.xls 재업로드) · <b>거래처관리(사업장)</b>(배송 점포, 발주 업로드 시 자동등록) · 회사/사용자 · 공통코드. <span style="color:#5a6b7a">회사/사용자·공통코드 관리는 <b>관리자 회사</b>(코네트)에만 보입니다. 매입/매출 거래처의 [＋ 거래처 추가] 창에도 <b>최근 등록 거래처코드·거래처명</b>과 다음 코드가 나옵니다.</span></td></tr>
