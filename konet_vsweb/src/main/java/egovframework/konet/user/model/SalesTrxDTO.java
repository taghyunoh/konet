package egovframework.konet.user.model;

import java.util.List;

/**
 * 판매 전표 헤더 DTO  →  TBL_SALES_TRX_MST
 *  · 전표 1건 = 헤더 1행(이 DTO) + 명세 N행(SalesTrxDtlDTO)
 *  · 매입등록(PurchaseDTO)과 대칭이다. 다른 칸은 custCd/custNm(매출처), dlvDt(납품일자), taxGb 뿐.
 *  · ★ 정산서(TBL_SALES_MST)와 다른 표다. 그쪽은 출고장이 준 엑셀 적재표라
 *    같은 (납품일자+출고장)을 재업로드하면 기존 행이 ACTION_YN='N' 으로 죽는다.
 *    여기는 정산서 밖에서 직접 판 건을 사람이 입력하는 전표라 재업로드와 무관하다.
 *  · 저장하면 명세마다 TBL_STOCK_LEDGER 출고행(REF_GB='SALE')이 파생 생성된다.
 *  · 전표번호 saleNo = 판매일자별 순번 '0001' (매입등록과 같은 체계)
 *  · 조회 파라미터(fromDt/toDt/findData/mgrCd)는 비영속 — 목록 검색용
 */
public class SalesTrxDTO {
    ...
    private Long    saleSeq;       // PK
    private String  saleDt;        // 판매일자 yyyymmdd (화면은 yyyy-mm-dd)
    private String  saleNo;        // 그날의 순번 '0001'
    private String  dlvDt;         // 납품일자 — 원장 귀속일. 비면 saleDt 로 본다
    private String  custCd;        // 매출처
    private String  custNm;
    private String  mgrCd;         // 담당사원
    private String  mgrNm;
    private String  whCd;          // 출고 창고
    private String  whNm;
    ...
    private Double  totAmt;        // 판매금액 합
    private Double  dcAmt;         // 할인액(헤더)
    private String  payGb;         // 현금/카드/외상 …
    private Double  payAmt;        // 전표에서 바로 받은 금액
    ...
    public String getSaleDt() { return saleDt; }
    public void setSaleDt(String saleDt) { this.saleDt = saleDt; }
    ...
    public List<SalesTrxDtlDTO> getItems() { return items; }
    public void setItems(List<SalesTrxDtlDTO> items) { this.items = items; }
