...
<style>
  ...
  .sbar input{ flex:1; height:42px; border:0; border-radius:10px; padding:0 12px; font-size:16px; min-width:0; color:var(--ink); }
  .sbar button{ height:42px; padding:0 16px; border:0; border-radius:10px; background:rgba(255,255,255,.18); color:#fff; font-size:15px; font-weight:700; }
  .opts{ display:flex; align-items:center; gap:14px; max-width:560px; margin:8px auto 0; font-size:13px; }
  .opts label{ display:flex; align-items:center; gap:6px; }
  .opts input{ width:17px; height:17px; accent-color:#fff; }
  .opts select{ height:30px; border:0; border-radius:8px; padding:0 8px; font-size:13px; }
  ...
  .pq{ font-size:12px; color:var(--mute); font-weight:500; }
  .neg{ color:var(--red) !important; }
  .dt{ display:grid; grid-template-columns:auto 1fr; gap:6px 12px; font-size:14px; }
  .dt dt{ color:var(--mute); font-weight:600; font-size:13px; }
  .dt dd{ margin:0; text-align:right; font-weight:700; font-variant-numeric:tabular-nums; word-break:break-all; }
  .io{ font-size:11.5px; font-weight:800; padding:1px 6px; border-radius:6px; margin-right:4px; }
  ...
  .sub-hint{ font-size:12.5px; color:var(--teal-d); background:var(--teal-l); border-radius:8px; padding:7px 10px; margin-bottom:8px; }
</style>
</head>
<body>
<header>
...
<main>
  <section class="card"><h2>상품 <small id="cnt"></small></h2><div id="body"><div class="empty">상품코드나 품명을 넣고 [조회] 하세요.<br><span style="font-size:12px">서브코드(매칭코드)로 쳐도 주코드로 찾아 줍니다.</span></div></div></section>
</main>
...
function search(){
  ...
  if(!q){ _alertBox('상품코드나 품명을 넣으세요.<br><span style="font-size:13px">전체 조회는 느려서 막아 두었습니다.</span>',{icon:'🔎'}); return; }
  _lastQ=q;
  ...
  loadAlias().then(function(al){
    ...
    el.innerHTML=h+'<div class="list cols2" style="margin-top:0">'+shown.map(function(o,i){
      ...
      return '<div class="row tap" onclick="detail('+i+')"><div class="k">'+esc(o.prodNm)
        +(stop?' <span class="badge gray" style="font-size:10.5px;padding:1px 6px">중지</span>':'')
...
function detail(i){
  ...
  var head='<div style="padding:4px 0 10px"><div style="font-size:16px;font-weight:800;line-height:1.35">'+esc(o.prodNm)+'</div>'
