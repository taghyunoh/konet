# VS Code 개발환경 가이드 (konet_vsapp — 모바일 PWA)

> **2026-09-11** 이 폴더는 `konet_vsweb`(PC 웹)을 통째로 복제한 **모바일(PWA) 앱**이다.
> 아래 톰캣·포트·WAR 이름은 **konet_vsapp 값**으로 바꿔 두었다(§6-5·§7-1 은 konet_vsweb 때의 기록이라 그대로 둠).

VS Code 설치 후 **서버 실행 · WAR 빌드 · Git 동기화**까지 한 번에 정리한 문서.
2026-07-23 실제 구축·검증한 내용 기준.

> **VS Code 자체가 처음이면** → [VSCode_처음_사용법.md](VSCode_처음_사용법.md) 를 먼저 볼 것
> (화면 구성·단축키·하루 작업 흐름·이클립스와 다른 점).
...
|---|---|---|
| 프로젝트 | `C:\Users\user\git\konet\konet_vsapp` | `C:\Users\HYUN\git\konet\konet_vsapp` |
...
| 톰캣 | `C:\egv\Servers\konet_vsapp-tomcat` | 동일 경로 |
| 웹앱 JAR | `C:\egv\Servers\konet_vsapp-vscode\lib` | 동일 경로 (113개, 3개 제외 §6-1) |
| HTTP / 셧다운 / 디버그 포트 | **9072 / 9014 / 9172** (PC 웹 konet_vsweb 은 9071 / 9013 / 9171) | 동일 |
| SERVERS 등록 이름 | `konet_vsapp` | 동일 |
| 세션 쿠키 이름 | `KONETAPP_SID` (web.xml — 9071 과 같은 브라우저에서 로그인이 서로 안 끊기게) | 동일 |
...
`konet_vsweb`(PC 웹)과 `konet_vsapp`(모바일 PWA)은 **독립된 사본**이다(2026-09-11 복제).
한쪽 수정이 다른 쪽에 반영되지 않으므로 **Java·SQL 은 두 곳 다 고치고, 고친 쪽에서 빌드**할 것.
WAR 이름도 다르다 — web = `konet_web-1.0.0.war` / app = **`konet_app-1.0.0.war`**(pom artifactId `konet_app`).
...
**File → Open Workspace from File** → `C:\Users\user\git\konet\konet.code-workspace`
(PC 웹 `konet_vsweb` 과 이 폴더가 한 창에 함께 뜬다. 이 폴더만 열려면 **File → Open Folder** → `C:\Users\user\git\konet\konet_vsapp`)
...
3. 폴더 → `C:\egv\Servers\konet_vsapp-tomcat`
4. `Apache Tomcat 8.5` 자동 감지 → Enter
5. **Server Name** 입력 (`konet_vsapp`) — 비우면 `Tomcat 8.5` 로 등록되고 **이름 변경 기능은 없다**(바꾸려면 Remove 후 재등록)
6. **`Required Attributes` 를 펼쳐** `server.home.dir` = `C:\egv\Servers\konet_vsapp-tomcat`
...
등록 후 우클릭 → **Edit Server** 에서 `server.http.port` 를 **9072** 로 고치고 `Ctrl+S`.
(기본값 8080 이라 그대로 두면 Open in Browser 가 엉뚱한 포트로 열린다)

> **지름길 (2호기에서 사용한 방법)**: 위저드 대신 등록 파일을 직접 만들어도 된다 —
> `C:\Users\<사용자>\.rsp\redhat-community-server-connector\servers\konet_vsapp` (확장자 없음, JSON)
> 에 `server.home.dir`/`server.base.dir`=`C:\egv\Servers\konet_vsapp-tomcat`, `server.http.port`=`9072`,
...
> (1호기의 `konet_vsapp` 등록도 2026-09-11 `konet_vsweb` 파일을 복사해 경로·포트만 바꿔 만들었다.)
> ⚠**RSP 는 켜질 때 한 번만 목록을 읽는다** — 파일을 넣은 뒤 목록에 안 보이면 `Community Server Connector` 우클릭 → **Start**(꺼져 있으면) 또는 Stop → Start.

### 3-2. 사용

서버 우클릭 → **Start / Stop / Restart / Debug**. 첫 기동 약 30초.
브라우저: http://localhost:9072/ (모바일 화면 = http://localhost:9072/m/index.do) · 로그: 우클릭 → **Show Output Channel**
...
`Ctrl+Shift+P` → `Tasks: Run Task` → **톰캣 시작 (konet_vsapp :9072)** / **톰캣 중지 (konet_vsapp)**
디버그는 **F5** → `톰캣 디버그 연결`.
...
탐색기 사이드바 **MAVEN** → `konet_app` → **Favorites** → 항목 옆 **▶**
...
결과물 **`target\konet_app-1.0.0.war`** (약 89MB, 45초~1분).
artifactId 가 `konet_app` 이라 **PC 웹(`konet_web-1.0.0.war`)과 파일명이 다르다** — 운영에 올릴 때 두 WAR 를 헷갈리지 말 것.
...
`C:\egv\Servers\konet_vsapp-vscode\lib` 에서 아래 3개를 **반드시 제외**한다.
...
```
robocopy <프로젝트경로>\target\konet_app-1.0.0\WEB-INF\lib C:\egv\Servers\konet_vsapp-vscode\lib /E /XF javaee-api-7.0.jar servlet-api-2.5.jar jsp-api-2.1.jar
```
...
셧다운 포트(이 앱은 9014) 바인딩 실패(`BindException`)로 **곧바로 자기 자신을 종료**한다. "시작됐는데 응답 없음" 으로 보인다.

```
Get-NetTCPConnection -State Listen -LocalPort 9014,9072 | Select-Object LocalPort,OwningProcess
```
...
5. 톰캣 복사본 만들기 — `apache-tomcat-8.5.66`(또는 이미 있는 `konet_vsweb-tomcat` 에서 logs·work·temp 뺀 복사) → **`C:\egv\Servers\konet_vsapp-tomcat`** → `conf\server.xml` 에 포트(**9072/9014**, redirect 9444)와 프로젝트 Context 지정(docBase=자기 PC의 `...\konet_vsapp\src\main\webapp`, PostResources = `...\konet_vsapp\target\classes` · `C:\egv\Servers\konet_vsapp-vscode\lib`), `conf\logging.properties`(EUC-KR)·`logging-utf8.properties`(UTF-8) 준비
6. 웹앱 JAR 채우기 — WAR 1회 빌드 후 `WEB-INF\lib` → `C:\egv\Servers\konet_vsapp-vscode\lib` 복사, §6-1 의 3개 제외
7. SERVERS 패널에 등록(§3-1, 이름 `konet_vsapp`) → Start → http://localhost:9072/ 로그인까지 확인
8. WAR 빌드 1회 검증(§4)
